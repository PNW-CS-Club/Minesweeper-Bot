...
public class Board {
   ...
   */
  public Board(int width, int height, int mines, List<WebElement> divs) {
    this.width = width;
    this.height = height;
    ...
    for (int i = 0; i < this.width * this.height; i++)
      this.board[i] = new Cell(divs.get(i));

    // set adjacent cells
    setBoardAdjacents();
  }

  public void updateBoard(List<WebElement> div) {
    int offset = 0;
    for (int i = 0; i < div.size(); ++i) {
      if (div.get(i).getAttribute("class").equals("clear")) {
        offset++;
        continue;
      }
      if (this.board[i - offset].getContents() != 'F')
        this.board[i - offset].updateCellContents(div.get(i - offset));
    }
  }
